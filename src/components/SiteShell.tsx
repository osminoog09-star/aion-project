...
const primaryNav = [
  ...
  { href: ecosystemRoutes.roadmap, label: "План" },
  { href: ecosystemRoutes.releases, label: "Скачать" },
] as const;
    ...
    <div className="aion-root flex min-h-full flex-col">
      ...
      <footer className="border-t border-white/10 bg-[#090b0d] px-4 py-8 text-center md:px-6">
        <nav className="mx-auto flex max-w-6xl flex-wrap justify-center gap-x-4 gap-y-2 text-[11px] text-slate-500">
          <Link href={ecosystemRoutes.roadmap} className="transition hover:text-cyan-400/90">
            План
          </Link>
          <Link href={ecosystemRoutes.aionProject} className="transition hover:text-cyan-400/90">
            Driver
          </Link>
          <Link href={ecosystemRoutes.releases} className="transition hover:text-cyan-400/90">
            Скачать
          </Link>
        </nav>
        <p className="mx-auto mt-4 max-w-2xl text-xs text-slate-400">
          AION Driver — приложение для водителя Bolt. Смена, доход, топливо, прибыль, карта.
        </p>
      </footer>
    </div>
  );
